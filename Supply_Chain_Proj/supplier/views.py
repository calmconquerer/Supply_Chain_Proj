...
def home(request):
    ...
    return render(request, 'supplier/base.html',
                  {'total_notification': total_notification, 'total_notification_supplier': total_notification_supplier,
                   'customer_row': customer_row, 'supplier_row': supplier_row,
...
def rfq_supplier(request):
    ...
    return render(request, 'supplier/rfq_supplier.html',
                  {'all_rfq': all_rfq, 'permission': permission, 'allow_customer_roles': allow_customer_roles,
...
def new_rfq_supplier(request):
    ...
    if request.method == 'POST':
        ...
        rfq_header = RfqSupplierHeader(rfq_no=get_last_rfq_no, date=date, attn=attn, follow_up=follow_up,
                                       footer_remarks=footer_remarks, account_id=account_id, company_id=company,
    ...
    return render(request, 'supplier/new_rfq_supplier.html',
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            for value in items:
                id = value["id"]
                id = Add_products.objects.get(id=id)
                rfq_detail = RfqSupplierDetail(item_id=id, quantity=value["quantity"], unit=value["unit"],
                                               rfq_id=header_id)
                rfq_detail.save()
            return JsonResponse({"result": "success"})
    except IntegrityError:
        print("Data Already Exist")
    return render(request, 'supplier/edit_rfq_supplier.html',
...
def quotation_supplier(request):
    ...
    return render(request, 'supplier/quotation_supplier.html',
                  {'all_quotation': all_quotation, 'permission': permission,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser})
...
def new_quotation_supplier(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return JsonResponse({"result": "No Account Found " + supplier + ""})
        date = datetime.date.today()
        quotation_header = QuotationHeaderSupplier(quotation_no=get_last_quotation_no, date=date, attn=attn,
                                                   prc_basis=prcbasis,
                                                   leadtime=leadtime, validity=validity, payment=payment,
        ...
        for value in items:
            id = value["id"]
            id = Add_products.objects.get(id=id)
            quotation_detail = QuotationDetailSupplier(item_id=id, quantity=value["quantity"], unit=value["unit"],
                                                       unit_price=value["unit_price"], remarks=value["remarks"],
                                                       quotation_id=header_id)
            quotation_detail.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'supplier/new_quotation_supplier.html',
...
def edit_quotation_supplier(request, pk):
    ...
    if item_code:
        item_id = Add_products.objects.get(product_code=item_code)
        item_code_exist = QuotationDetailSupplier.objects.filter(item_id=item_id,
                                                                 quotation_id=quotation_header.id).first()
        data = Add_products.objects.filter(id=item_id.id)
        if item_code_exist:
            return HttpResponse(json.dumps({'message': "Item Already Exist"}))
        row = serializers.serialize('json', data)
    ...
    if request.method == 'POST':
        ...
        for value in items:
            ...
            quotation_detail.save()
        return JsonResponse({"result": "success"})
    return render(request, 'supplier/edit_quotation_supplier.html',
...
def print_quotation_supplier(request, pk):
    ...
    pdf = render_to_pdf('supplier/quotation_supplier_pdf.html',
...
def quotation_export_supplier(request):
    ...
    columns = ['Quotation No', 'Date', 'Attn', 'Prc Basis', 'Lead Time', 'Validity', 'Payment', 'Remarks', 'Curreny',
               'Exchange Rate', 'Follow Up', 'Footer Remarks']
    ...
    rows = QuotationHeaderSupplier.objects.all().values_list('quotation_no', 'date', 'attn', 'prc_basis', 'leadtime',
                                                             'validity', 'payment', 'remarks', 'currency',
                                                             'exchange_rate', 'follow_up', 'footer_remarks')
    for row in rows:
        row_num += 1
...
def purchase_order_supplier(request):
    ...
    return render(request, 'supplier/purchase_order_supplier.html',
...
def new_purchase_order_supplier(request):
    ...
    if request.method == 'POST':
        ...
        for value in items:
            id = value["id"]
            id = Add_products.objects.get(id=id)
            po_detail = PoDetailSupplier(item_id=id, quantity=value["quantity"], unit=value["unit"],
                                         unit_price=value["unit_price"], remarks=value["remarks"],
                                         quotation_no="to be define", po_id=header_id)
            po_detail.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'supplier/new_purchase_order_supplier.html',
                  {'all_item_code': all_item_code, 'get_last_po_no': get_last_po_no, 'all_accounts': all_accounts,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser})
...
def edit_purchase_order_supplier(request, pk):
    ...
    if request.method == 'POST':
        ...
        for value in items:
            ...
            po_detail.save()
        return JsonResponse({"result": "success"})
    return render(request, 'supplier/edit_purchase_order_supplier.html',
...
def print_po_supplier(request, pk):
    ...
    pdf = render_to_pdf('supplier/po_supplier_pdf.html',
                        {'company_info': company_info, 'image': image, 'header': header, 'detail': detail,
                         'total_lines': total_lines, 'total_amount': total_amount,
                         'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                         'allow_transaction_roles': allow_transaction_roles,
                         'allow_inventory_roles': allow_inventory_roles,
                         'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser})
...
def delivery_challan_supplier(request):
    ...
    return render(request, 'supplier/delivery_challan_supplier.html',
...
def new_delivery_challan_supplier(request):
    ...
    if request.method == 'POST':
        ...
        date = datetime.date.today()
        dc_header = DcHeaderSupplier(dc_no=get_last_dc_no, date=date, footer_remarks=footer_remarks,
                                     follow_up=follow_up, account_id=account_id, company_id=company,
                                     user_id=request.user, branch_id=branch)
        dc_header.save()
        items = json.loads(request.POST.get('items'))
        ...
        for value in items:
            ...
            dc_detail = DcDetailSupplier(item_id=item_id, quantity=value["quantity"], accepted_quantity=0,
                                         returned_quantity=0, po_no="", dc_id=header_id)
            dc_detail.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'supplier/new_delivery_challan_supplier.html',
...
def edit_delivery_challan_supplier(request, pk):
    ...
    if request.method == 'POST':
        ...
        for value in items:
            ...
            item_id = Add_products.objects.get(id=value["id"])
            dc_detail = DcDetailSupplier(item_id=item_id, quantity=value["quantity"], accepted_quantity=0,
                                         returned_quantity=0, po_no="", dc_id=header_id, remarks=value["remarks"],
                                         unit=value["unit"])
            dc_detail.save()
        return JsonResponse({"result": "success"})
    return render(request, 'supplier/edit_delivery_challan_supplier.html',
...
def print_dc_supplier(request, pk):
    ...
    pdf = render_to_pdf('supplier/dc_supplier_pdf.html',
                        {'company_info': company_info, 'image': image, 'header': header, 'detail': detail,
                         'total_lines': total_lines, 'allow_customer_roles': allow_customer_roles,
...
def mrn_supplier(request):
    ...
    all_dc = DcHeaderSupplier.objects.all()
    return render(request, 'supplier/mrn_supplier.html',
                  {'all_dc': all_dc, 'permission': permission, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
                   'is_superuser': request.user.is_superuser})
