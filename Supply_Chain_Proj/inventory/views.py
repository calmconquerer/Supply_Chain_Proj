...
def item_stock(request):
    ...
    cursor.execute('''Select itemID,Size,item_code, item_name,Item_description,Unit,Size,SUM(quantity) As qty, main_category, sub_category From (
                    ...
                    ''')
    row = cursor.fetchall()
    return render(request, 'inventory/item_stock.html', {'row': row, 'allow_customer_roles': allow_customer_roles,
...
def add_product(request):
    ...
    if product_name and product_desc and type and size:
        ...
        return JsonResponse(
            {"product_name": product_name, "type": type, "size": size, "product_desc": product_desc, 'unit': unit,
             'opening_stock': opening_stock, 'main_category_id': main_category_id.id,
    ...
    if request.method == 'POST':
        ...
        for value in items:
            ...
            new_products = Add_products(product_code=item_code, product_name=value["item_name"],
...
def edit_item(request, pk):
    ...
    return render(request, 'inventory/edit_item.html',
                  {'all_detail': all_detail, 'pk': pk, 'allow_customer_roles': allow_customer_roles,
                   ...
                   'is_superuser': request.user.is_superuser, 'main_categories': main_categories,
...
def categories(request):
    ...
    main_categories = Category.objects.all()
    sub_categories = SubCategory.objects.all()
    return render(request, 'inventory/category_and_type.html',
                  {'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser,
                   'main_categories': main_categories, 'sub_categories': sub_categories})
