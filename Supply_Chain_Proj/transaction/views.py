...
from user.models import UserRoles, Branch
...
def purchase(request):
    ...
    print("here", company)
    all_purchases = PurchaseHeader.objects.filter(company_id=company, branch_id=branch).all()
    ...
    return render(request, 'transaction/purchase.html',
                  {'all_purchases': all_purchases, 'gst': gst, 'permission': permission,
...
def new_purchase(request):
    ...
    if request.method == 'POST':
        ...
        purchase_header = PurchaseHeader(purchase_no=purchase_id, date=date, footer_description=footer_desc,
                                         payment_method=payment_method, cartage_amount=cartage_amount,
                                         additional_tax=additional_tax, withholding_tax=withholding_tax,
                                         account_id=account_id, follow_up=follow_up, credit_days=credit_days,
        ...
        for value in items:
            ...
            purchase_detail = PurchaseDetail(item_id=item_id, quantity=value["quantity"], cost_price=value["price"],
                                             retail_price=0, sales_tax=value["sales_tax"], purchase_id=header_id,
                                             total=amount)
        ...
        if payment_method == 'Cash':
            tran2 = Transactions(refrence_id=header_id, refrence_date=date, account_id=account_id,
                                 tran_type="Purchase Invoice", amount=total_amount, date=date, remarks=purchase_id,
            ...
            tran1 = Transactions(refrence_id=header_id, refrence_date=date, account_id=cash_in_hand,
                                 tran_type="Purchase Invoice", amount=-abs(total_amount), date=date,
                                 remarks=purchase_id, ref_inv_tran_id=0, ref_inv_tran_type="", company_id=company,
                                 user_id=request.user, branch_id=branch)
            tran1.save()
        else:
            purchase_account = ChartOfAccount.objects.get(account_title='Purchases')
            tran1 = Transactions(refrence_id=header_id, refrence_date=date, account_id=account_id,
                                 tran_type="Purchase Invoice", amount=-abs(total_amount), date=date,
                                 remarks=purchase_id, ref_inv_tran_id=0, ref_inv_tran_type="", company_id=company,
                                 user_id=request.user, branch_id=branch)
            tran1.save()
            tran2 = Transactions(refrence_id=header_id, refrence_date=date, account_id=purchase_account,
                                 tran_type="Purchase Invoice", amount=total_amount, date=date, remarks=purchase_id,
                                 ref_inv_tran_id=0, ref_inv_tran_type="", company_id=company, user_id=request.user,
                                 branch_id=branch)
            tran2.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'transaction/new_purchase.html',
                  {'all_item_code': all_item_code, 'get_last_purchase_no': get_last_purchase_no,
...
def edit_purchase(request, pk):
    ...
    return render(request, 'transaction/edit_purchase.html',
                  {'all_item_code': all_item_code, 'all_accounts': all_accounts, 'purchase_header': purchase_header,
                   'purchase_detail': purchase_detail, 'pk': pk, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
                   'is_superuser': request.user.is_superuser})
...
def new_purchase_non_gst(request):
    ...
    company = Company_info.objects.get(id=company)
    branch = request.session['branch']
    branch = Branch(branch_id=branch)
    allow_customer_roles = customer_roles(request.user)
    allow_supplier_roles = supplier_roles(request.user)
    allow_transaction_roles = transaction_roles(request.user)
    allow_inventory_roles = inventory_roles(request.user)
    ...
    if request.method == 'POST':
        ...
        if follow_up:
            ...
            follow_up = '2010-06-10'
        purchase_header = PurchaseHeader(purchase_no=purchase_id, date=date, footer_description=footer_desc,
                                         payment_method=payment_method, cartage_amount=0.00, additional_tax=0.00,
                                         withholding_tax=0.00, account_id=account_id, follow_up=follow_up,
        ...
        for value in items:
            ...
            item_amount = item_amount + amount
            purchase_detail = PurchaseDetail(item_id=item_id, quantity=value["quantity"], cost_price=value["price"],
                                             retail_price=0, sales_tax=0.00, purchase_id=header_id, total=amount)
            purchase_detail.save()
        total_amount = item_amount
        header_id = header_id.id
        cash_in_hand = ChartOfAccount.objects.get(account_title='Cash')
        if payment_method == 'Cash':
            ...
            tran2.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'transaction/new_purchase_non_gst.html',
                  {'all_item_code': all_item_code, 'get_last_purchase_no': get_last_purchase_no,
                   'all_accounts': all_accounts, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
...
def edit_purchase_non_gst(request, pk):
    ...
    if request.method == 'POST':
        ...
        branch = request.session['branch']
        branch = Branch.objects.get(branch_id=branch)
        header_id = PurchaseHeader.objects.filter(company_id=company, branch_id=branch).get(purchase_no=purchase_id)
        ...
        for value in items:
            ...
            item_id = Add_products.objects.get(id=value["id"])
            purchase_detail = PurchaseDetail(item_id=item_id, quantity=value["quantity"], cost_price=value["price"],
                                             retail_price=0, sales_tax=0, purchase_id=header_id, total=amount)
            purchase_detail.save()
        ...
        if purchase_header.payment_method == 'Cash':
            ...
            delete.delete()
            tran2 = Transactions(refrence_id=header_id, refrence_date=date, account_id=account_id,
                                 tran_type="Purchase Invoice", amount=total_amount, date=date, remarks="Amount Debit",
                                 ref_inv_tran_id=0, ref_inv_tran_type="", company_id=company, user_id=request.user,
                                 branch_id=branch)
            tran2.save()
            tran1 = Transactions(refrence_id=header_id, refrence_date=date, account_id=cash_in_hand,
                                 tran_type="Purchase Invoice", amount=-abs(total_amount), date=date,
                                 remarks="Amount Debit", ref_inv_tran_id=0, ref_inv_tran_type="", company_id=company,
                                 user_id=request.user, branch_id=branch)
            tran1.save()
        else:
            refrence_id = Q(refrence_id=header_id)
            ...
            tran2.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'transaction/edit_purchase_non_gst.html',
                  {'all_item_code': all_item_code, 'all_accounts': all_accounts, 'purchase_header': purchase_header,
                   'purchase_detail': purchase_detail, 'pk': pk, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
...
def delete_purchase(request, pk):
    ...
    if item == True:
        ...
        messages.add_message(request, messages.ERROR,
                             "You cannot delete this Invoice, kindly delet it's voucher first.")
        return redirect('purchase')
...
def purchase_return_summary(request):
    ...
    branch = request.session['branch']
    branch = Branch.objects.get(branch_id=branch)
    company = Q(company_id=company, branch_code=branch)
    allow_customer_roles = customer_roles(request.user)
    allow_supplier_roles = supplier_roles(request.user)
    allow_transaction_roles = transaction_roles(request.user)
    allow_transaction_roles = transaction_roles(request.user)
    ...
    return render(request, 'transaction/purchase_return_summary.html',
                  {'all_purchase_return': all_purchase_return, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
                   'is_superuser': request.user.is_superuser})
...
def new_purchase_return(request, pk):
    ...
    if request.method == 'POST':
        ...
        date = datetime.date.today()
        account_id = ChartOfAccount.objects.get(account_title=supplier)
        purchase_return_header = PurchaseReturnHeader(purchase_no=get_last_purchase_no, date=date,
                                                      footer_description=description, payment_method=payment_method,
                                                      credit_days=credit_days, cartage_amount=cartage_amount,
                                                      additional_tax=additional_tax, withholding_tax=0,
        ...
        for value in items:
            print(value["id"])
            item_id = Add_products.objects.get(id=value["id"])
            purchase_return_detail = PurchaseReturnDetail(item_id=item_id, quantity=value["quantity"],
                                                          cost_price=value["price"], retail_price=0,
                                                          sales_tax=value["sales_tax"], purchase_return_id=header_id)
        ...
        if purchase_header.payment_method == 'Cash':
            ...
            company = Company_info.objects.get(id=company)
            tran2 = Transactions(refrence_id=header_id, refrence_date=date, account_id=account_id,
                                 tran_type="Purchase Return Invoice", amount=-abs(total_amount), date=date,
                                 remarks="Amount Debit", copmany_id=company, user_id=request.user, branch_id=branch)
            tran2.save()
            tran1 = Transactions(refrence_id=header_id, refrence_date=date, account_id=cash_in_hand,
                                 tran_type="Purchase Return Invoice", amount=total_amount, date=date,
                                 remarks="Amount Debit", copmany_id=company, user_id=request.user, banch_id=branch)
            tran1.save()
        else:
            company = request.session['company']
            company = Company_info.objects.get(id=company)
            ...
            tran1 = Transactions(refrence_id=0, refrence_date=date, account_id=account_id, tran_type="",
                                 amount=total_amount, date=date, remarks="Amount Debit", ref_inv_tran_id=header_id,
                                 ref_inv_tran_type="Purchase Return Invoice", user_id=request.user, copmany_id=company,
                                 branch_id=branch)
            tran1.save()
            tran2 = Transactions(refrence_id=0, refrence_date=date, account_id=purchase_account, tran_type="",
                                 amount=-abs(total_amount), date=date, remarks="Amount Debit",
                                 ref_inv_tran_id=header_id, ref_inv_tran_type="Purchase Return Invoice",
                                 user_id=request.user, copmany_id=company, branch_id=branch)
            tran2.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'transaction/purchase_return.html',
                  {'purchase_header': purchase_header, 'purchase_detail': purchase_detail, 'pk': pk,
                   'get_last_purchase_no': get_last_purchase_no, 'permission': permission,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser})
...
def edit_purchase_return(request, pk):
    ...
    if request.method == 'POST':
        ...
        for value in items:
            item_id = Add_products.objects.get(id=value["id"])
            purchase_return_detail = PurchaseReturnDetail(item_id=item_id, quantity=value["quantity"],
                                                          cost_price=value["price"], retail_price=0.00,
                                                          sales_tax=value["sales_tax"], purchase_return_id=header_id)
            purchase_return_detail.save()
            quantity = float(value["quantity"])
            price = float((value["price"]))
        ...
        if purchase_header.payment_method == 'Cash':
            ...
            tran2 = Transactions(refrence_id=header_id, refrence_date=date, account_id=account_id,
                                 tran_type="Purchase Return Invoice", amount=-abs(total_amount), date=date,
                                 remarks="Amount Credit", company=company, user_id=request.user, branch_id=branch)
            tran2.save()
            tran1 = Transactions(refrence_id=header_id, refrence_date=date, account_id=cash_in_hand,
                                 tran_type="Purchase Return Invoice", amount=total_amount, date=date,
                                 remarks="Amount Debit", company=company, user_id=request.user, branch_id=branch)
            tran1.save()
        else:
            purchase_account = ChartOfAccount.objects.get(account_title='Purchase Returns')
            tran1 = Transactions(refrence_id=header_id, refrence_date=date, account_id=account_id,
            ...
            tran2.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'transaction/edit_purchase_return.html',
                  {'purchase_header': purchase_header, 'purchase_detail': purchase_detail, 'pk': pk,
                   'all_accounts': all_accounts, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
...
def sale(request):
    ...
    return render(request, 'transaction/sale.html',
                  {'all_sales': all_sales, 'permission': permission, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
                   'is_superuser': request.user.is_superuser, 'gst': gst})
...
def new_sale(request):
    ...
    if request.method == 'POST':
        ...
        if follow_up:
            ...
            follow_up = '2010-06-10'
        sale_header = SaleHeader(sale_no=sale_id, date=date, footer_description=footer_desc,
                                 payment_method=payment_method, cartage_amount=cartage_amount,
                                 additional_tax=additional_tax, withholding_tax=withholding_tax, account_id=account_id,
                                 follow_up=follow_up, credit_days=credit_days, company_id=company, user_id=request.user,
                                 hs_code=hs_code, branch_id=branch)
        cart = json.loads(request.POST.get('cartage'))
        items = json.loads(request.POST.get('items'))
        sale_header.save()
        ...
        if payment_method == 'Cash':
            ...
            tran1 = Transactions(refrence_id=header_id, refrence_date=date, account_id=account_id,
                                 ...
                                 ref_inv_tran_id=0, ref_inv_tran_type="", company_id=company, user_id=request.user)
            tran1.save()
            tran2 = Transactions(refrence_id=header_id, refrence_date=date, account_id=sale_account,
                                 tran_type="Sale Invoice", amount=-abs(total_amount), date=date, remarks=sale_id,
                                 ref_inv_tran_id=0, ref_inv_tran_type="", company_id=company, user_id=request.user)
            tran2.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'transaction/new_sales.html',
                  {'get_last_sale_no': get_last_sale_no, 'all_accounts': all_accounts, 'all_dc': all_dc,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser})
...
def direct_sale(request, pk):
    ...
    if request.method == 'POST':
        ...
        sale_header = SaleHeader(sale_no=sale_id, date=date, footer_description=footer_desc,
                                 payment_method=payment_method, cartage_amount=0.00, additional_tax=additional_tax,
                                 withholding_tax=0.00, account_id=account_id, follow_up='2010-06-10',
                                 company_id=company, user_id=request.user, hs_code=hs_code)
        items = json.loads(request.POST.get('items'))
        ...
        if payment_method == 'Cash':
            ...
            tran2.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'transaction/direct_invoice.html',
                  {'all_item_code': all_item_code, 'get_last_sale_no': get_last_sale_no, 'all_accounts': all_accounts,
                   'dc_header': dc_header, 'dc_detail': dc_detail, 'pk': pk,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser})
...
def edit_sale(request, pk):
        ...
        try:
            ...
            if get_account:
                customer_dc = cursor.execute('''Select Distinct id,dc_no,accountid,account_title From (
                                            ...
                                            Inner Join customer_dcheadercustomer HD on  HD.id = tblData.dc_id_id
                                            Where RemainingQuantity > 0 AND accountid = %s AND HD.company_id_id = %s ''',
                                             [get_account.id, company.id])
                customer_dc = customer_dc.fetchall()
    ...
    if request.method == 'POST':
        ...
        for value in items:
            ...
            item_amount = item_amount + amount
            sale_detail = SaleDetail(item_id=item_id, quantity=value["quantity"], cost_price=value["price"],
                                     retail_price=0, sales_tax=value["sales_tax"], sale_id=header_id,
                                     dc_ref=value["dc_no"], dcdetailid=value["dcdetailid"], total=amount)
            sale_detail.save()
        item_amount = item_amount + float(additional_tax)
        total_amount = item_amount
        header_id = header_id.id
        ...
        if sale_header.payment_method == 'Cash':
            ...
            delete = Transactions.objects.filter(refrence_id, tran_type)
            delete.delete()
            tran1 = Transactions(refrence_id=header_id, refrence_date=sale_header.date, account_id=cash_in_hand,
                                 tran_type="Sale Invoice", amount=total_amount, date=date, ref_inv_tran_id=0,
                                 ref_inv_tran_type="", remarks=sale_id, company_id=company, user_id=request.user)
            tran1.save()
            tran2 = Transactions(refrence_id=header_id, refrence_date=sale_header.date, account_id=account_id,
                                 tran_type="Sale Invoice", amount=-abs(total_amount), date=date, remarks=sale_id,
                                 ref_inv_tran_id=0, ref_inv_tran_type="", company_id=company, user_id=request.user)
            tran2.save()
        else:
            ...
            delete.delete()
            sale_account = ChartOfAccount.objects.get(account_title='Sales')
            tran1 = Transactions(refrence_id=header_id, refrence_date=sale_header.date, account_id=account_id,
                                 tran_type="Sale Invoice", amount=total_amount, date=date, remarks=sale_id,
                                 ref_inv_tran_id=0, ref_inv_tran_type="", company_id=company, user_id=request.user)
            tran1.save()
            tran2 = Transactions(refrence_id=header_id, refrence_date=sale_header.date, account_id=sale_account,
                                 tran_type="Sale Invoice", amount=-abs(total_amount), date=date, remarks=sale_id,
                                 ref_inv_tran_id=0, ref_inv_tran_type="", company_id=company, user_id=request.user)
            tran2.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'transaction/edit_sale.html',
                  {'all_item_code': all_item_code, 'all_accounts': all_accounts, 'sale_header': sale_header,
                   'sale_detail': sale_detail, 'pk': pk, 'all_dc': all_dc, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
                   'is_superuser': request.user.is_superuser, 'cartages': cartages})
...
def new_sale_non_gst(request):
    ...
    if request.method == 'POST':
        ...
        for value in items:
            ...
            item_id = Add_products.objects.get(id=value["id"])
            sale_detail = SaleDetail(item_id=item_id, quantity=value["quantity"], cost_price=value["price"],
                                     retail_price=0, sales_tax=0, dc_ref=value["dc_no"], dcdetailid=value["dcdetailid"],
                                     sale_id=header_id, total=amount)
            sale_detail.save()
        total_amount = item_amount
        header_id = header_id.id
        total_amount = total_amount + cartage_sum
        ...
        if payment_method == 'Cash':
            ...
            tran2.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'transaction/new_sale_non_gst.html',
                  {'get_last_sale_no': get_last_sale_no, 'all_accounts': all_accounts, 'all_dc': all_dc,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser})
...
def direct_sale_non_gst(request, pk):
    ...
    if request.method == 'POST':
        ...
        sale_header = SaleHeader(sale_no=sale_id, date=date, footer_description=footer_desc,
                                 payment_method=payment_method, cartage_amount=0.00, additional_tax=0.00,
                                 withholding_tax=0.00, account_id=account_id, follow_up=date, credit_days="60",
        ...
        if payment_method == 'Cash':
            ...
            tran2.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'transaction/direct_invoice_non_gst.html',
                  {'all_item_code': all_item_code, 'get_last_sale_no': get_last_sale_no, 'all_accounts': all_accounts,
                   'dc_header': dc_header, 'dc_detail': dc_detail, 'pk': pk,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
...
def edit_sale_non_gst(request, pk):
    ...
    if request.method == 'POST':
        ...
        for value in items:
            ...
            item_amount = item_amount + amount
            sale_detail = SaleDetail(item_id=item_id, quantity=value["quantity"], cost_price=value["price"],
                                     retail_price=0, sales_tax=0, sale_id=header_id, dc_ref=value["dc_no"],
                                     dcdetailid=value["dcdetailid"], total=amount)
            sale_detail.save()
        total_amount = item_amount
        header_id = header_id.id
        total_amount = total_amount + cartage_sum
        ...
        if sale_header.payment_method == 'Cash':
            ...
            tran2.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'transaction/edit_sale_ngst.html',
                  {'all_item_code': all_item_code, 'all_accounts': all_accounts, 'sale_header': sale_header,
                   'sale_detail': sale_detail, 'pk': pk, 'all_dc': all_dc, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
...
def sale_return_summary(request):
    ...
    return render(request, 'transaction/sale_return_summary.html',
                  {'all_sales_return': all_sales_return, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
                   'is_superuser': request.user.is_superuser})
...
def new_sale_return(request, pk):
    ...
    if request.method == 'POST':
        ...
        date = datetime.date.today()
        account_id = ChartOfAccount.objects.get(account_title=customer)
        sale_return_header = SaleReturnHeader(sale_no=get_last_sale_no, date=date, footer_description=description,
                                              payment_method=payment_method, cartage_amount=cartage_amount,
                                              additional_tax=additional_tax, withholding_tax=0, account_id=account_id,
                                              company_id=company, user_id=request.user)
        ...
        for value in items:
            ...
            sale_return_detail = SaleReturnDetail(item_id=item_id, quantity=value["quantity"],
                                                  cost_price=value["price"], retail_price=0,
                                                  sales_tax=value["sales_tax"], sale_return_id=header_id)
        ...
        if sale_header.payment_method == 'Cash':
            tran1 = Transactions(refrence_id=header_id, refrence_date=date, account_id=cash_in_hand,
                                 tran_type="Sale Return Invoice", amount=-abs(total_amount), date=date, remarks="",
                                 ref_inv_tran_id=0, ref_inv_tran_type="", company_id=company, user_id=request.user)
            tran1.save()
            tran2 = Transactions(refrence_id=header_id, refrence_date=date, account_id=account_id,
                                 tran_type="Sale Return Invoice", amount=total_amount, date=date, remarks="",
                                 ref_inv_tran_id=0, ref_inv_tran_type="", company_id=company, user_id=request.user)
            tran2.save()
        else:
            sale_return_account = ChartOfAccount.objects.get(account_title='Sales Returns')
            tran1 = Transactions(refrence_id=0, refrence_date=date, account_id=account_id, tran_type="",
                                 amount=-abs(total_amount), date=date, remarks="", ref_inv_tran_id=header_id,
                                 ref_inv_tran_type="Sale Invoice Return", company_id=company, user_id=request.user)
            tran1.save()
            tran2 = Transactions(refrence_id=0, refrence_date=date, account_id=sale_return_account, tran_type="",
                                 amount=total_amount, date=date, remarks="", ref_inv_tran_id=header_id,
                                 ref_inv_tran_type="Sale Invoice Return", company_id=company, user_id=request.user)
            tran2.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'transaction/sale_return.html',
                  {'sale_header': sale_header, 'sale_detail': sale_detail, 'pk': pk,
                   'get_last_sale_no': get_last_sale_no, 'permission': permission,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser})
...
def edit_sale_return(request, pk):
    ...
    branch = request.session['branch']
    branch = Branch.objects.get(branch_id=branch)
    company = Q(company_id=company, branch_i=branch)
    allow_customer_roles = customer_roles(request.user)
    allow_supplier_roles = supplier_roles(request.user)
    allow_transaction_roles = transaction_roles(request.user)
    allow_inventory_roles = inventory_roles(request.user)
    ...
    if request.method == 'POST':
        ...
        if sale_header.payment_method == 'Cash':
            ...
            tran1 = Transactions(refrence_id=header_id, refrence_date=date, account_id=account_id,
                                 ...
                                 company_id=company, user_id=request.user, branch_id=branch)
            tran1.save()
            tran2 = Transactions(refrence_id=header_id, refrence_date=date, account_id=sale_return_account,
                                 tran_type="Sale Return Invoice", amount=total_amount, date=date, remarks="",
                                 company_id=company, user_id=request.user, branch_id=branch)
            tran2.save()
        return JsonResponse({'result': 'success'})
    return render(request, 'transaction/edit_sale_return.html',
                  {'sale_header': sale_header, 'sale_detail': sale_detail, 'pk': pk, 'all_accounts': all_accounts,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser})
...
def reports(request):
    ...
    allow_inventory_roles = inventory_roles(request.user)
    all_accounts = ChartOfAccount.objects.all()
    return render(request, 'transaction/reports.html',
                  {'all_accounts': all_accounts, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
                   'is_superuser': request.user.is_superuser})
...
def journal_voucher_summary(request):
    ...
    return render(request, 'transaction/journal_voucher_summary.html',
                  {'all_voucher': all_voucher, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
...
def journal_voucher(request):
    ...
    if request.method == "POST":
        ...
        if invoice_id:
            ...
            doc_no = doc_no
        date = datetime.date.today()
        jv_header = VoucherHeader(voucher_no=get_last_tran_id, date=date, doc_no=doc_no, doc_date=doc_date,
        ...
        for value in items:
            ...
            if value["credit"] > "0" and value["credit"] > "0.00":
                ...
                jv_detail2.save()
        return JsonResponse({"result": "success"})
    return render(request, 'transaction/journal_voucher.html',
                  {"all_invoices": all_invoices, "all_accounts": all_accounts, 'get_last_tran_id': get_last_tran_id,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'permission': permission,
                   'is_superuser': request.user.is_superuser})
...
def edit_journal_voucher(request, pk):
    ...
    return render(request, 'transaction/edit_journal_voucher.html',
                  {"all_invoices": all_invoices, "all_accounts": all_accounts, 'jv_header': jv_header,
                   'jv_detail': jv_detail, 'pk': pk, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
                   'is_superuser': request.user.is_superuser})
...
def jv_pdf(request, pk):
    ...
    pdf = render_to_pdf('transaction/jv_pdf.html',
                        {'company_info': company_info, 'header': header, 'detail': detail, "debit": debit,
                         "credit": credit})
    if pdf:
...
def view_cash_receiving(request, pk):
    ...
    return render(request, 'transaction/view_cash_receiving_voucher.html',
                  {'voucher_header': voucher_header, 'voucher_detail': voucher_detail,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser})
...
def new_cash_receiving_voucher(request):
    ...
    cursor = connection.cursor()
    get_last_tran_id = cursor.execute(
        '''select * from transaction_voucherheader where voucher_no LIKE '%CRV%' order by voucher_no DESC LIMIT 1  ''')
    get_last_tran_id = get_last_tran_id.fetchall()
    ...
    if request.method == "POST":
        ...
        for value in items:
            ...
            tran1 = Transactions(refrence_id=0, refrence_date=doc_date, tran_type='', amount=amount,
                                 ...
                                 ref_inv_tran_type="Sale CRV", voucher_id=voucher_id.id, company_id=company,
                                 user_id=request.user)
            tran1.save()
            ...
            jv_detail1 = VoucherDetail(account_id=cash_account, debit=amount, credit=0.00, header_id=header_id,
                                       invoice_id=invoice_no.id)
            jv_detail1.save()
            jv_detail2 = VoucherDetail(account_id=account_id, debit=0.00, credit=-abs(amount), header_id=header_id,
                                       invoice_id=invoice_no.id)
            jv_detail2.save()
        return JsonResponse({"result": "success"})
    return render(request, 'transaction/new_cash_receiving_voucher.html',
                  {"all_accounts": all_accounts, 'all_invoices': all_invoices, 'get_last_tran_id': get_last_tran_id,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser})
...
def view_bank_receiving(request, pk):
    ...
    voucher_header = VoucherHeader.objects.filter(company, id=pk).first()
    voucher_detail = VoucherDetail.objects.filter(header_id=header_id.id).all()
    return render(request, 'transaction/view_bank_receiving_voucher.html',
                  {'voucher_header': voucher_header, 'voucher_detail': voucher_detail,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser})
...
def new_bank_receiving_voucher(request):
    ...
    if request.method == "POST":
        ...
        for value in items:
            ...
            jv_detail2.save()
        return JsonResponse({"result": "success"})
    return render(request, 'transaction/new_bank_receiving_voucher.html',
...
def view_bank_payment(request, pk):
    ...
    voucher_header = VoucherHeader.objects.filter(company, id=pk).first()
    voucher_detail = VoucherDetail.objects.filter(header_id=header_id.id).all()
    return render(request, 'transaction/view_bank_payment_voucher.html',
                  {'voucher_header': voucher_header, 'voucher_detail': voucher_detail,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser})
    ...
    if request.method == "POST":
        ...
        for value in items:
            ...
            jv_detail1 = VoucherDetail(account_id=bank_account, debit=0.00, credit=-abs(amount), header_id=header_id,
                                       invoice_id=invoice_no.id)
            jv_detail1.save()
            jv_detail2 = VoucherDetail(account_id=account_id, debit=amount, credit=0.00, header_id=header_id,
                                       invoice_id=invoice_no.id)
            jv_detail2.save()
        return JsonResponse({"result": "success"})
    return render(request, 'transaction/new_bank_payment_voucher.html',
                  {"all_accounts": all_accounts, 'get_last_tran_id': get_last_tran_id,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser,
                   'all_bank': all_bank, 'all_invoices': all_invoices})
...
def new_cash_payment_voucher(request):
    ...
    return render(request, 'transaction/new_cash_payment_voucher.html',
                  {"all_accounts": all_accounts, 'get_last_tran_id': get_last_tran_id, 'all_invoices': all_invoices,
                   'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser})
...
def crv_pdf(request, pk):
    ...
    pdf = render_to_pdf('transaction/crv_pdf.html', {'company_info': company_info, 'header': header, 'detail': detail,
...
def cpv_pdf(request, pk):
    ...
    detail = detail.fetchall()
    amount_in_words = num2words(abs(detail[0][0]))
    pdf = render_to_pdf('transaction/cpv_pdf.html', {'company_info': company_info, 'header': header, 'detail': detail,
                                                     'amount_in_words': amount_in_words})
    if pdf:
        response = HttpResponse(pdf, content_type='application/pdf')
        filename = "CashReceivingVoucher.pdf"
...
def bpv_pdf(request, pk):
    ...
    pdf = render_to_pdf('transaction/bpv_pdf.html', {'company_info': company_info, 'header': header, 'detail': detail,
                                                     'amount_in_words': amount_in_words, 'bank': bank})
    if pdf:
        response = HttpResponse(pdf, content_type='application/pdf')
        filename = "CashReceivingVoucher.pdf"
        content = "inline; filename='%s'" % (filename)
...
def brv_pdf(request, pk):
    ...
    pdf = render_to_pdf('transaction/brv_pdf.html', {'company_info': company_info, 'header': header, 'detail': detail,
                                                     'amount_in_words': amount_in_words, 'bank': bank})
    if pdf:
        response = HttpResponse(pdf, content_type='application/pdf')
        filename = "CashReceivingVoucher.pdf"
...
def account_ledger(request):
    ...
    if request.method == "POST":
        ...
        id = account_id.id
        pdf = render_to_pdf('transaction/account_ledger_pdf.html',
                            {'ledger_list': ledger_list, 'company_info': company_info, 'image': image, 'row': row,
                             'debit_amount': debit_amount, 'credit_amount': credit_amount,
                             'account_title': account_title, 'from_date': from_date, 'to_date': to_date, 'id': id})
        if pdf:
            response = HttpResponse(pdf, content_type='application/pdf')
...
def trial_balance(request):
    ...
    if request.method == "POST":
        ...
        pdf = render_to_pdf('transaction/trial_balance_pdf.html',
                            {'company_info': company_info, 'row': row, 'debit_amount': debit_amount,
                             'credit_amount': credit_amount, 'from_date': from_date, 'to_date': to_date,
                             'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
    ...
    if request.method == "POST":
        ...
        for v in row:
            if v[2] >= 0:
                ...
                credit_amount = credit_amount + v[2]
        pdf = render_to_pdf('transaction/trial_balance_pdf.html',
                            {'company_info': company_info, 'image': image, 'row': row, 'debit_amount': debit_amount,
                             'credit_amount': credit_amount, 'allow_customer_roles': allow_customer_roles,
...
def sale_detail_item_wise(request):
    ...
    if request.method == "POST":
        ...
        for value in row:
            total = total + value[3]
        print(total)
        pdf = render_to_pdf('transaction/sale_detail_item_wise_pdf.html',
                            {'company_info': company_info, 'row': row, 'from_date': from_date, 'to_date': to_date,
                             'total': total, 'allow_customer_roles': allow_customer_roles,
                             'allow_supplier_roles': allow_supplier_roles,
                             'allow_transaction_roles': allow_transaction_roles,
                             'allow_inventory_roles': allow_inventory_roles,
                             'allow_report_roles': report_roles(request.user),
...
def sale_summary_item_wise(request):
    ...
    if request.method == "POST":
        ...
        account_title = account_id.account_title
        pdf = render_to_pdf('transaction/sale_summary_item_wise_pdf.html',
                            {'company_info': company_info, 'image': image, 'row': row, 'from_date': from_date,
                             'to_date': to_date, 'total': total, 'all_accounts': all_accounts,
                             'account_title': account_title, 'allow_customer_roles': allow_customer_roles,
                             'allow_supplier_roles': allow_supplier_roles,
                             'allow_transaction_roles': allow_transaction_roles,
                             'allow_inventory_roles': allow_inventory_roles,
                             'allow_report_roles': report_roles(request.user),
...
def sales_tax_invoice(request, pk):
    ...
    pdf = render_to_pdf('transaction/sales_tax_invoice_pdf_lines.html',
                        {'company_info': company_info, 'header': header, 'detail': detail, 'total_lines': total_lines,
                         'total_amount_item': total_amount_item, 'allow_customer_roles': allow_customer_roles,
                         'allow_supplier_roles': allow_supplier_roles,
                         'allow_transaction_roles': allow_transaction_roles,
                         'allow_inventory_roles': allow_inventory_roles, 'is_superuser': request.user.is_superuser,
                         'parent_company_name': parent_company_name, 'tax_amount': tax_amount,
                         'total_amount': total_amount, 'hs_code': hs_code, 'vog': vog,
                         'allow_report_roles': report_roles(request.user)})
    if pdf:
        response = HttpResponse(pdf, content_type='application/pdf')
...
def commercial_invoice(request, pk):
    ...
    lines = lines + len(detail) + len(detail)
    total_lines = 36 - lines
    pdf = render_to_pdf('transaction/commercial_invoice_pdf_lines.html',
                        {'company_info': company_info, 'header': header, 'detail': detail, 'total_lines': total_lines,
                         'total_amount_item': total_amount_item, 'allow_customer_roles': allow_customer_roles,
                         'allow_supplier_roles': allow_supplier_roles,
                         'allow_transaction_roles': allow_transaction_roles,
                         'allow_inventory_roles': allow_inventory_roles, 'is_superuser': request.user.is_superuser,
                         'parent_company_name': parent_company_name, 'cartage_amounts': cartage_amounts,
                         'tax_amount': tax_amount, 'vog': vog, 'total_amount': total_amount, 'hs_code': hs_code,
                         'allow_report_roles': report_roles(request.user)})
    if pdf:
        response = HttpResponse(pdf, content_type='application/pdf')
        filename = "SaleTaxInvoice%s.pdf" % (header.sale_no)
...
def commercial_invoice_non_gst(request, pk):
    ...
    lines = lines + len(detail) + len(detail)
    total_lines = 36 - lines
    pdf = render_to_pdf('transaction/commercial_invoice_pdf_non_gst.html',
                        {'company_info': company_info, 'header': header, 'detail': detail, 'total_lines': total_lines,
                         'total_amount_item': total_amount_item, 'allow_customer_roles': allow_customer_roles,
                         'allow_supplier_roles': allow_supplier_roles,
                         'allow_transaction_roles': allow_transaction_roles,
                         'allow_inventory_roles': allow_inventory_roles, 'is_superuser': request.user.is_superuser,
                         'parent_company_name': parent_company_name, 'cartage_amounts': cartage_amounts,
                         'total_amount': total_amount, 'allow_report_roles': report_roles(request.user)})
    if pdf:
        response = HttpResponse(pdf, content_type='application/pdf')
        filename = "SaleTaxInvoice%s.pdf" % (header.sale_no)
        content = "inline; filename='%s'" % (filename)
...
def multi_companies(request):
    ...
    return render(request, 'transaction/multi_companies.html',
                  {'all_companies': all_companies, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
                   'is_superuser': request.user.is_superuser})
...
def multi_branches(request):
    ...
    return render(request, 'transaction/multi_branches.html',
                  {'all_branches': all_branches, 'allow_customer_roles': allow_customer_roles,
                   'allow_supplier_roles': allow_supplier_roles, 'allow_transaction_roles': allow_transaction_roles,
                   'allow_inventory_roles': allow_inventory_roles, 'allow_report_roles': report_roles(request.user),
                   'is_superuser': request.user.is_superuser})
...
def new_multi_companies(request):
    ...
    if request.method == 'POST':
        ...
        stn = request.POST.get('stn')
        cnic = request.POST.get('cnic')
        new_companies = Company_info(company_name=company_name, company_address=company_address,
                                     company_type=company_type, phone_no=phone_no, email=email_address,
                                     website=web_site, ntn=ntn, stn=stn, cnic=cnic)
        new_companies.save()
...
@login_required
@user_passes_test(Is_superuser)
def new_multi_branches(request):
    company = request.session['company']
    company = Company_info.objects.get(id=company)
    ...
    branch_code = cursor.execute(
        '''SELECT * FROM user_branch WHERE company_id_id = %s ORDER by branch_id DESC LIMIT 1''', [company.id])
    branch_code = branch_code.fetchall()
    if branch_code:
        branch_code = branch_code[0][1]
    ...
    if request.method == 'POST':
        ...
        branch_phone_no = request.POST.get('branch_phone_no')
        branch_mobile_no = request.POST.get('branch_mobile_no')
        branch_email = request.POST.get('branch_email')

        new_branches = Branch(branch_name=branch_name, branch_address=branch_address, branch_code=branch_code,
                              branch_phone_no=branch_phone_no, branch_mobile_no=branch_mobile_no,
                              branch_email=branch_email, company_id=company)
        new_branches.save()

    return render(request, 'transaction/new_multi_branch.html',
                  {'allow_customer_roles': allow_customer_roles, 'allow_supplier_roles': allow_supplier_roles,
                   'allow_transaction_roles': allow_transaction_roles, 'allow_inventory_roles': allow_inventory_roles,
                   'allow_report_roles': report_roles(request.user), 'is_superuser': request.user.is_superuser,
...
def edit_multi_branches(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f'Branch Profile has been updated!')
